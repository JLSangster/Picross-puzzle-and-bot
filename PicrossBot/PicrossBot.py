...
class puzzleSolver:
    ...
    def phaseOne(self, puzzleModel):
        print("Phase One")
        print("Final comumn is " + str(puzzleModel.workingClues[puzzleModel.size - 1][1]))
        ...
        for y in range(puzzleModel.size):
            ...
            if np.any(puzzleModel.workingClues[y][0] != 0):
                ...
                if highClue == highest:
                    ...
                    if start + highest < puzzleModel.size:
    ...
    def phaseThree(self, puzzleModel):
        ...
        for x in range(puzzleModel.size):
            ...
            for y in range(puzzleModel.size):
                ...
                elif (puzzleModel.grid[y][x] != "empty") and (gapSize[gapCount] != 0):
                    gapCount += 1
                    gapSize.append(0)
                    gapLoc.append(0)
            ...
            largeClue = 0
            #with a gap found
            #if the gap is the same size as the largest clue, it can be filled
            ...
            #the gaps are contextual to each other they can't just be considered in isolation.
            #find if x is the *only* gap it could go into

            #identify if there is more than one gap the largest clue could fit in
            #if there is only one item in gapSize are equal to or greater than largeClue
            input("Testing")
            gapSize = np.asarray(gapSize)
            
            #mark gaps smaller than the smallest clue
            if (len(np.where(gapSize < smallClue)[0]) >= 1):
                for i in range(len(np.where(gapSize < smallClue)[0])):
                    gap = np.where(gapSize < smallClue)[0][i]
                    fillable = False
                    if gapSize[gap] > 0:
                        fillable = True
                
                    if (gapLoc[gap] -1 > -1):
                        if (puzzleModel.grid[gapLoc[gap] - 1][x] == "corr"):
                            fillable = False #(served by a different case)

                    if (gapLoc[gap] + gapSize[gap] < puzzleModel.size):                  
                        if (puzzleModel.grid[gapLoc[gap] + gapSize[gap]][x] == "corr"):
                            fillable = False #(served by a different case)
                    if fillable:
                        self.fillCells(self, puzzleModel, gapSize[gap], x, gapLoc[gap], False, False)

            if (len(np.where(gapSize >= largeClue)[0]) == 1):
                i = np.where(gapSize >= largeClue)[0][0]
                #temporarily check its fillable
                ...
                #then do whatever to fix it
                if fillable:
                    print("fillable ---")
                    print(gapSize[i])
                    #A gap bordered by anything other than correct cells can be treated as if it were an empty row
                    #if the clue is equal to the size 
                    if (largeClue == gapSize[i]) and largeClue > 0:
                        print("large match")
                        print(gapLoc[i])
                        #fill that clue
                        self.fillCells(self, puzzleModel, largeClue, x, gapLoc[i], False, True)

                    if (largeClue > (gapSize[i] / 2)) and largeClue > 0:
                        print("half match")
                        fillNum = gapSize[i] - largeClue
                        self.fillCells(self, puzzleModel, gapSize[i] - (fillNum * 2) + gapLoc[i], x, fillNum, False, True)
                else:
                    print("Not fillable")
            ...
            #            if len(np.where(puzzleModel.workingClues[x][0] > 0)) == 1:
            #                #if the clue is equal to the size 
            #                if (largeClue == gapSize[i]) and largeClue > 0:
            #                    print("large match")
            #                    print(gapLoc[i])
        ...
        #        elif (puzzleModel.grid[y][x] != "empty") and (gapSize != 0):
        #            break
                #continue through to the end of the line
